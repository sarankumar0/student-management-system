...
const BatchTable = ({ students,refreshData }) => {
  ...
  {isModalOpen && (
        <div className="fixed inset-0 flex items-center h-screen justify-center bg-black bg-opacity-50">
...
const Reports = () => {
  ...
  const searchedStudents = filteredStudents.filter(
    ...
    const downloadFile = async (format,searchedStudents) => {
        ...
        if (format === "csv") {
          const csv = Papa.unparse(searchedStudents);
  ...
  <div className="text-lg font-semibold text-indigo-700">
    Report of Registerion
  </div>

  {/* Right-aligned Dropdown and Search */}
  <div className="flex me-5 items-center gap-4">
    <select
      className="w-56 border border-gray-300 rounded-lg pl-3 pr-4 py-2 focus:ring-2 focus:ring-indigo-400 outline-none shadow-sm transition"
      value={selectedBatch}
    ...
    <div className="relative w-72">
      ...
      />
                  ...
                  <div className="relative group">
                   <button onClick={() => downloadFile("csv", searchedStudents)}
                   ...
                   <span className="text-sm text-white">Download as Excel</span>
                   </button>

                {/* Tooltip */}
               <div className="absolute -top-7 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs 
                  px-3 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity 
                  whitespace-nowrap">Download searched/selected students</div>
                </div>
